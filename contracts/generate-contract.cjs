...
const schedule = new Table({
  ...
  rows: [
    ...
    new TableRow({
      children: ['금요일', '17시 30분', '21시 30분', '30분 (19:30~20:00)', '3시간 30분']
        .map((t, i) => cell(t, schedCols[i])),
    }),
    new TableRow({
      children: ['토요일', '17시 30분', '21시 30분', '30분 (19:30~20:00)', '3시간 30분']
        .map((t, i) => cell(t, schedCols[i])),
    }),
    new TableRow({
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      schedule,
      para('※ 1주 소정근로시간: 10시간 (휴게시간 제외)', { before: 100, indent: { left: 300 } }),
      ...
      clauseTitle('7. 특약사항'),
      para('① 2026년 7월 17일(금)은 예외적으로 13시 30분부터 19시 30분까지 근무한 것으로 하며(휴게 30분 포함, 실근로 5시간 30분), 해당 근로에 대한 임금은 제5조에 따라 지급한다.', { indent: { left: 300 } }),
